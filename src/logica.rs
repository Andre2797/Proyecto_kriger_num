...
pub fn num_letras(line :String)->String{
    ...
    if(str3.len()==3){
      ...
      res= nom_centenas(num);
       ...
       let mut second_centena:String=String::from("");
       let mut first_centena:String=String::from("");
       let mut millones_letr:String=String::from("");
       let mut millones_Arg:String=String::from("");
    ...
    for i in 0..chars.len(){
        ...
        if(i==chars.len()){
         ...
         first_centena= nom_centenas(num);
        }else if( i%2!=0){
            let mut  num=chars[i].parse::<usize>().unwrap();
             millones_letr=nom_centenas(num); 
            let mut miles= num % 100000000000;
            let mut millones_num= num - miles;
            let mut obtner_millones=(millones_num/100000000000);
           millones_Arg=millones[obtner_millones].to_string();
        ...
        let mut  num=chars[i].parse::<usize>().unwrap();
     second_centena=nom_centenas(num) +" mil ";


    }
  return  millones_letr +&second_centena+ &millones_Arg + &first_centena;
    
   

//let mut centena1=vec![];
//nom_mil(str3.to_string());
}
let position=chars.len()-1;
let mut  num=chars[position].parse::<usize>().unwrap();
         
//return millones_letr + &millones_Arg+&second_centena + &nom_centenas(num);
